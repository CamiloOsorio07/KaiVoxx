...
async def send_now_playing_embed(bot, song):
    ...
    embed.add_field(name="Duration", value="0:00", inline=False)
...
async def update_now_playing_bar(bot, guild_id, song):
    ...
    while True:
        ...
        embed = msg.embeds[0]
        embed.set_field_at(2, name="Duration", value=f"{elapsed//60:02}:{elapsed%60:02}", inline=False)
        try:
            await msg.edit(embed=embed)
        except Exception:
            break
        await asyncio.sleep(5)  # Update every 10 seconds to avoid rate limiting
